...
def tts():
    ...
    wavs = chat.infer(
        ...
        params_refine_text={"prompt": prompt, "max_new_token": refine_max_new_token},
    ...
    filename = (
        ...
        + f"use{inference_time_rounded}s-audio{audio_duration_rounded}s-seed{voice}-te{temperature}-tp{top_p}-tk{top_k}-textlen{len(text)}-{str(random())[2:7]}"
        + ".wav"
    )
